...
async fn list_groups(Query(q): Query<ListQuery>) -> impl IntoResponse {
    let result = with_db(|conn| {
        let (sql, params): (String, Vec<Box<dyn rusqlite::types::ToSql>>) = match &q.tenant_id {
            Some(tid) => (
                "SELECT id, tenant_id, name, description FROM groups WHERE tenant_id = ?1"
                    .to_string(),
...
async fn create_group(Json(body): Json<CreateGroup>) -> impl IntoResponse {
    ...
    match result {
        Ok(_) => {
            ...
            (
                StatusCode::CREATED,
                Json(json!({"id": id, "name": body.name, "tenant_id": body.tenant_id})),
            )
                .into_response()
        }
        Err(e) => {
            ...
            if msg.contains("UNIQUE") {
                (
                    StatusCode::CONFLICT,
                    Json(json!({"error": "group with this name already exists in tenant"})),
                )
                    .into_response()
            } else {
                (
...
async fn delete_group(Path(id): Path<String>) -> impl IntoResponse {
    let result = with_db(|conn| conn.execute("DELETE FROM groups WHERE id = ?1", [&id]));
    ...
    match result {
        ...
        Err(e) => {
            ...
            if msg.contains("UNIQUE") || msg.contains("PRIMARY") {
                (
                    StatusCode::CONFLICT,
                    Json(json!({"error": "user already in group"})),
                )
                    .into_response()
            } else if msg.contains("FOREIGN") {
                (
                    StatusCode::BAD_REQUEST,
                    Json(json!({"error": "user or group does not exist"})),
                )
                    .into_response()
            } else {
                (
