...
async fn create_tenant(Json(body): Json<CreateTenant>) -> impl IntoResponse {
    ...
    match result {
        Ok(_) => {
            ...
            (
                StatusCode::CREATED,
                Json(json!({"id": id, "name": body.name, "slug": body.slug})),
            )
                .into_response()
        }
        Err(e) => {
            ...
            if msg.contains("UNIQUE") {
                (
                    StatusCode::CONFLICT,
                    Json(json!({"error": "tenant with this slug already exists"})),
                )
                    .into_response()
            } else {
                (
...
async fn update_tenant(
    ...
    match result {
        Ok(0) => (
            ...
            Json(json!({"error": "tenant not found or no changes"})),
        )
            .into_response(),
        Ok(_) => {
...
async fn delete_tenant(Path(id): Path<String>) -> impl IntoResponse {
    let result = with_db(|conn| conn.execute("DELETE FROM tenants WHERE id = ?1", [&id]));
